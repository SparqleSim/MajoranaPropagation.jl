...
function HybridGate(nfermionicsites::Int, f_symbol::Symbol, f_sites::Vector{Int}, 
    ...
    return HybridGate(rotations, nfermionicsites, nqubits, is_spinful, truncate_after_each_hybrot)
end

#No input for is_spinful => defaultes to false
HybridGate(nfermionicsites::Int, f_symbol::Symbol, f_sites::Vector{Int}, nqubits::Int, q_symbols::Vector{Symbol}, q_sites::Vector{Int}) = HybridGate(nfermionicsites, f_symbol, f_sites, nqubits, q_symbols, q_sites, false)
HybridGate(nfermionicsites::Int, f_symbol::Symbol, f_sites::Vector{Int}, nqubits::Int, is_spinful::Bool) = HybridGate(nfermionicsites, f_symbol, f_sites, nqubits, Symbol[], Int[], is_spinful) 
HybridGate(nfermionicsites::Int, f_symbol::Symbol, f_sites::Vector{Int}, nqubits::Int) = HybridGate(nfermionicsites, f_symbol, f_sites, nqubits, Symbol[], Int[], false)
HybridGate(nfermionicsites::Int, nqubits::Int, q_symbols::Vector{Symbol}, q_sites::Vector{Int}, is_spinful::Bool) = HybridGate(nfermionicsites, :n, Int[], nqubits, q_symbols, q_sites, is_spinful)
HybridGate(nfermionicsites::Int, nqubits::Int, q_symbols::Vector{Symbol}, q_sites::Vector{Int}) = HybridGate(nfermionicsites, :n, Int[], nqubits, q_symbols, q_sites, false)
...
function PauliPropagation.PropagationBase.applytoall!(gate::HybridRotation, prop_cache::HybridPropagationCache, parameter; kwargs...)
    #=
    Overload
    ...
    P_gate = gate.term & kwargs[:qubits_filter]
    mu_gate = gate.term & kwargs[:fermions_filter]

    for (hs, coeff) in hsum  
        P_string = hs & kwargs[:qubits_filter]
        mu_string = hs & kwargs[:fermions_filter]
...
function PauliPropagation.PropagationBase.applymergetruncate!(h_gate::HybridGate, prop_cache::AbstractHybridPropagationCache, 
                                                                theta; truncate_each_hr=nothing, kwargs...)
    hsum = mainsum(prop_cache)
    aux_hsum = auxsum(prop_cache)
...
function PauliPropagation.apply(gate::HybridGate, hstr, coeff, parameter; kwargs...)
    ...
    =#
    makeshift_hsum = HybridSum(gate.nfermionicsites, gate.nqubits, gate.is_spinful)
