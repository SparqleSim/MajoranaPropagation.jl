...
let
    ...
    for k = 1:n_reps
        ...
        # normal 
        #@timeit to "normal" propagate!(circ_single, msum, thetas_single; min_abs_coeff=min_abs_coeff, max_unpaired=max_singles, to)

        # vector
        @timeit to "vec" times_vec[k] = @elapsed propagate!(circ_single, vec_msum, thetas_single; min_abs_coeff=min_abs_coeff, max_unpaired=max_singles, to)
        #println("time vec: $(print_time(times_vec[k]))")

        # multi-vector
        @timeit to "multivec" times_multivec[k] = @elapsed propagate!(circ_single, multivec_msum, thetas_single; min_abs_coeff=min_abs_coeff, max_unpaired=max_singles, to)
